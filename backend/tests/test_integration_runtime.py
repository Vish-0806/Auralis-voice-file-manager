"""Unit tests for IntegrationRuntime and singleton accessors (Phase 11.9)."""

import threading
# pyrefly: ignore [missing-import]
import pytest

from brain.os.integration import (
    IntegrationProvider,
    IntegrationRuntime,
    IntegrationStatus,
    OperationRequest,
    OperationResponse,
    OperationTarget,
    get_integration_runtime,
    reset_integration_runtime,
)


def test_integration_runtime_lifecycle() -> None:
    rt = IntegrationRuntime()
    assert rt.get_health().state == "Initializing"

    rt.initialize()
    assert rt.get_health().state == "Running"

    provider = rt.get_provider()
    assert isinstance(provider, IntegrationProvider)

    req = OperationRequest(target=OperationTarget.FILESYSTEM, capability="filesystem.open")
    resp = rt.execute(req)
    assert isinstance(resp, OperationResponse)

    rt.shutdown()
    assert rt.get_health().state == "Stopped"


def test_integration_runtime_singleton() -> None:
    reset_integration_runtime()
    rt1 = get_integration_runtime()
    rt2 = get_integration_runtime()

    assert rt1 is rt2
    assert rt1.get_health().state == "Running"

    reset_integration_runtime()
    rt3 = get_integration_runtime()
    assert rt3 is not rt1


def test_integration_runtime_thread_safety() -> None:
    reset_integration_runtime()
    rt = get_integration_runtime()

    def worker() -> None:
        for _ in range(50):
            req = OperationRequest(target=OperationTarget.FILESYSTEM, capability="filesystem.open")
            rt.execute(req)
            rt.get_statistics()
            rt.get_health()

    threads = [threading.Thread(target=worker) for _ in range(10)]
    for t in threads:
        t.start()
    for t in threads:
        t.join()

    assert rt.get_health().state == "Running"
